  ...
  include_once $_SESSION["directorioPaterno"].'angoschool/areaDireccaoProvincial/manipulacaoDadosDoAjax.php';
    ...
    class manipulacaoDadosDoAjaxInterno extends manipulacaoDadosAjax{
      ...
      function __construct($caminhoAbsoluto){
        ...
        $this->tipoPacoteEscola = trim(filter_input(INPUT_POST, "tipoPacoteEscola", FILTER_SANITIZE_STRING));
        ...
        if($this->accao=="salvarEscola"){
            if($this->verificacaoAcesso->verificarAcessoAlteracao(["aGestGPE"], "", "", "FNão tens permissão de cadastrar nova escola.")){
               $this->salvarEscola();
            }
        }else if($this->accao=="editarEscola"){
            if($this->verificacaoAcesso->verificarAcessoAlteracao(["aGestGPE"], "", "", "FNão tens permissão de editar dados duma escola.")){
                $this->editarEscola();
            }
        }else if($this->accao=="excluirEscola"){
             if($this->verificacaoAcesso->verificarAcessoAlteracao(["aGestGPE"], "", "", "FNão tens permissão de excluir uma escola.")){
                $this->excluirEscola();
             }
        }        
      ...
      private function salvarEscola(){
        $jaExistemNumero="V";
            ...
            if($this->inserir("escolas", "nomeEscola, endereco, estadoEscola, numeroInternoEscola, tituloEscola, privacidadeEscola, nivelEscola, pais, provincia, municipio, comuna, tipoInstituicao, periodosEscolas, abrevNomeEscola, abrevNomeEscola2, tipoPacoteEscola, corCabecalhoTabelas, corLetrasCabecalhoTabelas, alturaCartEstudante, tamanhoCartEstudante, corCart1, corCart2, corLetrasCart, corBordasCart, criterioEscolhaTurno", [$this->nomeEscola, $this->enderecoEscola, $this->estadoEscola, $numeroUnico, $this->tituloEscola, $this->privacidade, $this->nivelEscola, valorArray($this->sobreUsuarioLogado, "pais"), valorArray($this->sobreUsuarioLogado, "provincia"), $this->municipio, $this->comuna, $this->tipoInstituicao, $this->periodosEscolas, $this->abrevNomeEscola, $this->abrevNomeEscola2, $this->tipoPacoteEscola, "#ffc000", "#000000", "210.00", "800.00", "#7c0644", "#fff2cc", "#000000", "#ffc000", $this->criterioEscolhaTurno])=="sim"){
              ...
              $idAnoActual =$this->selectUmElemento("anolectivo", "idPAno", "estado=:estado", ["V"]);

              $this->inserir("ano_escola", "idAnoEscola, idFAno, estadoAnoL", [$idPEscola, $idAnoActual, "V"]);

              $this->inserir("anexos", "idAnexoEscola, identidadeAnexo", [$idPEscola, "Sede"]);

              $this->inserir("acessoareas", "idAcessoEscola, nomeArea, acessoVisualizacao, acessoAlteracao, chaveArea", [$idPEscola, "aPedagogica", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "aPedagogica-".$idPEscola]);

              $this->inserir("acessoareas", "idAcessoEscola, nomeArea, acessoVisualizacao, acessoAlteracao, chaveArea", [$idPEscola, "aAdministrativa", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "aAdministrativa-".$idPEscola]);
              $this->inserir("acessoareas", "idAcessoEscola, nomeArea, acessoVisualizacao, acessoAlteracao, chaveArea", [$idPEscola, "aDirectoria", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "aDirectoria-".$idPEscola]);
              $this->inserir("acessoareas", "idAcessoEscola, nomeArea, acessoVisualizacao, acessoAlteracao, chaveArea", [$idPEscola, "aComissaoPais", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "Administrativo,Pedagógico,Director,Chefe_da_Secretaria,Promotor", "aComissaoPais-".$idPEscola]);
      ...
      private function editarEscola(){
        if($this->editar("escolas", "nomeEscola, endereco, estadoEscola, tituloEscola, privacidadeEscola, nivelEscola, pais, provincia, municipio, comuna, tipoInstituicao, periodosEscolas, abrevNomeEscola, abrevNomeEscola2, tipoPacoteEscola, criterioEscolhaTurno", [$this->nomeEscola, $this->enderecoEscola, $this->estadoEscola, $this->tituloEscola, $this->privacidade, $this->nivelEscola, valorArray($this->sobreUsuarioLogado, "pais"), valorArray($this->sobreUsuarioLogado, "provincia"), $this->municipio, $this->comuna, $this->tipoInstituicao, $this->periodosEscolas, $this->abrevNomeEscola, $this->abrevNomeEscola2, $this->tipoPacoteEscola, $this->criterioEscolhaTurno], "idPEscola=:idPEscola", [$this->idPEscola])=="sim"){
          $this->listar();
        }else{
      ...
      private function listar(){
        echo $this->selectJson("escolas LEFT JOIN div_terit_comunas ON idPComuna=comuna LEFT JOIN div_terit_municipios ON idPMunicipio=municipio", "*", "tipoInstituicao=:tipoInstituicao AND provincia=:provincia AND idPEscola!=4 AND privacidadeEscola=:privacidadeEscola", ["escola", valorArray($this->sobreUsuarioLogado, "provincia"), "Pública"], "nomeEscola ASC");
      }
    }
