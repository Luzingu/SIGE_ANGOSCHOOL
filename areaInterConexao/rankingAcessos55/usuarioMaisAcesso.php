<?php session_start();  
    ...
    includar("../../");
    $manipulacaoDados = new manipulacaoDados("Usuário com mais acesso.");
  ...
  <section id="main-content"> 
          ...
          <div class="row">
            <div class="col-lg-12 col-md-12">
              <nav role="navigation" class="navbar navbar-inverse paternSubMenuInterno" > 
                  ...
                  <h1 class="lead navbar-brand" style="color: white;"><strong><i class="fa fa-star"></i> Usuários com Mais Acessos</strong></h1>
              </nav>
            </div>
        </div>
        <div class="main-body">
        <?php  if($verificacaoAcesso->verificarAcesso(11, ["qualquerAcesso"], array(), "msg")){
         ...
         ?>
                      ...
                      <?php 
                          ...
                          foreach ($datasEntradas as $data) {
                            ...
                            if($i==count($datasEntradas)){ 
                              $idDataFinal =$manipulacaoDados->selectUmElemento("entidadesonline", "idPOnline", ["dataEntrada"=>$data], "idPOnline DESC");
                            }
                            echo "<option value='".$manipulacaoDados->selectUmElemento("entidadesonline", "idPOnline", ["dataEntrada"=>$data], "idPOnline DESC")."'>".converterData($data)."</option>";
                          }
          ...
          <?php
            $arrayFinal=array();
             $idDataInicial = (int)$idDataInicial;
             $idDataFinal = (int)$idDataFinal;
            ...
            $entidadesOnline = $manipulacaoDados->anexarTabela($entidadesOnline, "alunosmatriculados", "idPMatricula", "idOnlineMat");
            $entidadesOnline = $manipulacaoDados->anexarTabela($entidadesOnline, "entidadesprimaria", "idPEntidade", "idOnlineEnt");
            $entidadesOnline = $manipulacaoDados->anexarTabela($entidadesOnline, "escolas", "idPEscola", "idOnlineEntEscola");
            
            
            foreach ($manipulacaoDados->selectDistinct("entidadesonline", "idOnlineEnt", ["idPOnline"=>array('$gte'=>$idDataInicial), "idPOnline"=>array('$lte'=>$idDataFinal)]) as $acesso) {
              
              $arrayFinal[] = totalDeTempo($entidadesOnline, $acesso);                                             
            }
            foreach ($manipulacaoDados->selectDistinct("entidadesonline", "idOnlineMat", ["idPOnline"=>array('$gte'=>$idDataInicial), "idPOnline"=>array('$lte'=>$idDataFinal)]) as $acesso) {
               $arrayFinal[] = totalDeTempo($entidadesOnline, "", $acesso);                                            
            }
            ...
            foreach ($arrayFinal as $ar) {
              if($ar["nomeUsuario"]!="" && $ar["nomeUsuario"]!=null){
                $i++;
                $ar["ordem"]=($i);
                if($i<=$topAcessos){
                ...
                <table class="table table-striped table-bordered table-hover" >
                    <thead class="corPrimary">
                      <tr>
                        ...
                        <th class="lead" style="width: 300px;"><strong>Escola</strong></th>
...
<?php $conexaoFolhas->folhasJs(); $janelaMensagens->funcoesDaJanelaJs(); $includarHtmls->formTrocarSenha();
  ...
  function totalDeTempo($entidadesOnline, $idOnlineEnt="", $idOnlineMat=""){
    ...
    foreach ($entidadesOnline as $entidade) {
      ...
      if(($idOnlineEnt!="" && $idOnlineMat=="" && nelson($entidade, "idOnlineEnt")==$idOnlineEnt) || ($idOnlineMat!="" && $idOnlineEnt=="" && nelson($entidade, "idOnlineMat")==$idOnlineMat)){
    ...
    lista.filter(condition).forEach(function(dado){
      ...
      if(contagem>=paginacao.comeco && contagem<=paginacao.final){
        ...
        if(dado.tipoUsuario=="Aluno"){
          link ="<a href='"+caminhoRecuar+"../areaAdministrador/perfilAluno?aWRQTWF0cmljdWxh="+dado.idUsuario+"' class='black'>";
        }else{
          link ="<a href='"+caminhoRecuar+"../areaAdministrador/perfilEntidade?aWRQUHJvZmVzc29y="+dado.idUsuario+"' class='black'>";
        }
