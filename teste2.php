<?php 
    ...
    $array = $m->selectArray("alunos_2", $campos, ["pautas.update"=>['$in'=>array('2024-01-04', '2024-01-05')]], ["pautas"]);
    ...
    foreach($array as $p)
    {
        ...
        if($strings!="")
        {
            $nota = $m->selectArray("alunos_2", ["pautas.mtI"], ["pautas.chavePauta"=>$p["pautas"]["chavePauta"]], ["pautas"], 1);
            if(valorArray($nota, "mtI", "pautas") != $p["pautas"]["mtI"])
            {
                $m->editarItemObjecto("alunos_2", "pautas", $strings, $valores, ["idPMatricula"=>$p["idPMatricula"]], ["chavePauta"=>$p["pautas"]["chavePauta"]]);
            }
        }
    }
?>