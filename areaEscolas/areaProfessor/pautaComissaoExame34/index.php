<?php
    ...
    includar("../../");
    $manipulacaoDados = new manipulacaoDados("Pauta da Comissão de Exame", "pautaComissaoExame");
...
<body>
  ...
  <section id="main-content"> 
     ...
     <div class="row" >
              ...
              <h1 class="lead navbar-brand" style="color: white;"><strong><i class="fa fa-print"></i> <?php 
                echo "MINI-PAUTA(S) DE EXAME(S)"; ?><br>
              </strong></h1> 
            </nav>
            </div>
          </div>
      ...
      <div class="col-lg-5 col-md-5 lead" id="pesqUsario">
        Disciplinas
          <select id="referenciaDisciplina" name="referenciaDisciplina" class="form-control">
           <?php 
              foreach ($manipulacaoDados->selectArray("divisaoprofessores", ["classe", "sePorSemestre", "designacaoTurmaDiv", "nomeDisciplina", "abrevCurso", "abreviacaoDisciplina2", "periodoTurmaDiv", "semestre", "idPNomeDisciplina", "idPNomeCurso", "nomeTurmaDiv", "avaliacoesContinuas"], ["idPEscola"=>$_SESSION['idEscolaLogada'], "idDivAno"=>$manipulacaoDados->idAnoActual, "idPresidenteComissaoExame"=>$_SESSION['idUsuarioLogado']]) as $divisao){
                ...
                {                
                  if(($divisao["classe"]==6 || $divisao["classe"]==9 || $divisao["classe"]==12 || (nelson($divisao, "sePorSemestre")=="sim" && valorArray($sobreDisciplina, "continuidadeDisciplina", "disciplinas")=="T")) && !(nelson($divisao, "tipoCurso")=="pedagogico" && ($divisao["idPNomeDisciplina"]==51 || $divisao["idPNomeDisciplina"]==140))){
...
<?php $conexaoFolhas->folhasJs(); $janelaMensagens->funcoesDaJanelaJs(); $includarHtmls->dataList(); $includarHtmls->formTrocarSenha();
 ...
 <form id="formularioDados">
    ...
    <input type="hidden" name="trimestre" id="trimestre" value="IV">
