<?php
    ...
    class segundoCiclo{
        ...
        function __construct($db){
            ...
            $this->db->arrayAlunosInscritos = $this->db->selectArray("alunosmatriculados", ["idPMatricula", "dataNascAluno", "sexoAluno", "reconfirmacoes.nomeTurma", "reconfirmacoes.idReconfAno", "reconfirmacoes.classeReconfirmacao", "reconfirmacoes.idReconfEscola"], ["reconfirmacoes.idReconfAno"=>$this->db->idPAno, "reconfirmacoes.idReconfEscola"=>$_SESSION['idEscolaLogada'], "escola.idMatCurso"=>array('$in'=>$arrayCursos), "escola.idMatEscola"=>$_SESSION['idEscolaLogada'], "reconfirmacoes.estadoReconfirmacao"=>"A"], ["escola"], "", [], [], $this->db->matchMaeAlunos($this->db->idPAno));
        }
        ...
        public function visualizar(){
            ...
            $htmlRetorno ="<div style='position: absolute;'><div style='margin-top: 0px; width:300px;'>".$this->db->assinaturaDirigentes(7)."</div></div>".$this->db->cabecalho()."<p style='".$this->db->text_center.$this->db->maiuscula.$this->db->bolder."'>ALUNOS REPETENTES - ".$this->db->numAno."</p></div>";
            ...
            foreach ($this->db->selectArray("nomecursos", [], ["cursos.idCursoEscola"=>$_SESSION['idEscolaLogada'], "cursos.estadoCurso"=>"A", "tipoCurso"=>$this->db->tipoCurso], ["cursos"], "", [], ["nomeCurso"=>1]) as $curso){
                ...
                $htmlRetorno .="<tr><td style='".$this->db->bolder.$this->db->border().$this->db->maiuscula."'>".$curso["nomeCurso"]."</td>";

                foreach($cabecalho as $cab){
                    $htmlRetorno .=$this->db->contador($cab["tituloDb"], "TOT", "TOT", "TOT", "TOT").$this->db->contador($cab["tituloDb"], "F", $curso["idPNomeCurso"], "TOT", "TOT");
                }
                $htmlRetorno .="</tr>";
            }
