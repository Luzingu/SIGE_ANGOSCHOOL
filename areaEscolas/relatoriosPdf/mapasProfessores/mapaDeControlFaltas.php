    ...
    class mapaForcaTrabalho extends funcoesAuxiliares{
        ...
        function __construct($caminhoAbsoluto){
         ...
         private function mapa(){
            ...
            $this->controlFaltas = $this->selectArray("entidadesprimaria", ["controlPresenca.implicacao", "controlPresenca.faltas", "idPEntidade"], ["escola.idEntidadeEscola"=>$_SESSION['idEscolaLogada'], "escola.estadoActividadeEntidade"=>"A", "escola.tipoPessoal"=>"docente", "controlPresenca.idEscola"=>$_SESSION['idEscolaLogada'], "controlPresenca.data"=>array('$gte'=>$this->diaInicial), "controlPresenca.faltas"=>array('$gt'=>0), "controlPresenca.data"=>array('$lte'=>$this->dataFinal)], ["escola", "controlPresenca"]);
            ...
            <p style='".$this->text_center.$this->bolder.$this->maiuscula.$this->miniParagrafo."'>Mapa de FALTAS REFERENTE AO mês de ".nomeMes($this->mesFinal)." de ".$this->anoCivil."<p/>
            ...
            <table style='".$this->tabela."font-size:9pt; width:100%;'>
              <tr style='".$this->corDanger.$this->bolder.$this->text_center."'>
                ...
                $this->html .="<td style='".$this->border()."' rowspan='2'>Total</td><td style='".$this->border()."' rowspan='2'>N.º<br>Faltas</td></tr>
                <tr style='".$this->corDanger.$this->bolder.$this->text_center."'>";
                foreach($datas as $data){
                    ...
                    foreach($datas as $data){
                        ...
                        foreach($this->controlFaltas as $control){
                            if($control["idPEntidade"]==$entidade["idPEntidade"] && $control["controlPresenca"]["data"]==$data["data"]){
                                ...
                                $faltas = valorArray($control, "faltas", "controlPresenca");
                                $contAusencias +=intval(valorArray($control, "faltas", "controlPresenca"));

                                if(valorArray($control, "implicacao", "controlPresenca")=="T"){
                                    $contFaltas++;
                                }else{
                                    $contAusenciasSemImplicacao +=intval(valorArray($control, "faltas", "controlPresenca"));
                                }
                                break;
                            }
                        }
                        $this->html .="<td style='".$this->border().$this->text_center."'>".$faltas."</td>";
                        
                    }
                    $contFaltas += intdiv($contAusenciasSemImplicacao, 6);
                    $this->html .="<td style='".$this->border().$this->text_center."'>".$contAusencias."</td><td style='".$this->border().$this->text_center."'>".$contFaltas."</td></tr>";
                }
