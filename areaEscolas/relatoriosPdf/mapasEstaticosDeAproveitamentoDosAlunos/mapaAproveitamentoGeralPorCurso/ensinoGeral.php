    ...
    class mapa extends funcoesAuxiliares{
        ...
        private function exbirMapaPorTurma(){
           ...
           foreach (listarItensObjecto($this->sobreCurso, "classes") as $classe) {
               ...
               foreach ($this->turmasEscola([intval($this->idPCurso)], [intval($classe["identificador"])], $this->idPAno) as $turma) {
                     
                    $this->html .="<tr><td style='".$this->border().$this->text_center."'>".$turma["designacaoTurma"]."</td>"; 
                    foreach ($this->analizador->cabecalhos as $cab) {
                        
                        $this->html .="<td style='".$this->border().$this->text_center."'>".$this->analizador->quanto($this->idPCurso, $classe["identificador"], $turma["nomeTurma"], $cab["tituloDb"], $cab["genero"])."</td>";
                    }
                    $this->html .="</tr>";
               }
               ...
               $this->html .="<tr><td style='".$this->border().$this->text_center.$this->bolder."'>SUBTOTAL</td>";
                    foreach ($this->analizador->cabecalhos as $cab) {
                        $this->html .="<td style='".$this->border().$this->text_center.$this->bolder."'>".$this->analizador->quanto($this->idPCurso, $classe["identificador"], "TOT", $cab["tituloDb"], $cab["genero"])."</td>";
                    }
                    $this->html .="</tr>";
           }
           ...
           <div style='".$this->text_center."'>".$this->assinaturaDirigentes("mengi")."</div>";    
        ...
        private function exbirMapaPorClasse(){
           ...
           foreach (listarItensObjecto($this->sobreCurso, "classes") as $classe) {
               ...
               $this->html .="<tr><td style='".$this->border().$this->text_center.$this->maiuscula."'>".$classe["designacao"]."</td>";
                foreach ($this->analizador->cabecalhos as $cab) {
                    $this->html .="<td style='".$this->border().$this->text_center."'>".$this->analizador->quanto($this->idPCurso, $classe["identificador"], "TOT", $cab["tituloDb"], $cab["genero"])."</td>";
                }
                $this->html .="</tr>";
           }
